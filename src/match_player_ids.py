...
def searchPlayer(first_name, last_name, players):
    return players.loc[(players["first_name"] == first_name.lower().replace("-", " ")) & 
                         (players["last_name"].str.contains(last_name.lower()))]

def matchPlayer(p_id, full_name, players, player_ids, new_players_to_scrap_ids):
    if p_id not in player_ids:       
        my_man = None
        matched = re.search("(.*)\ (.+)$", full_name)
        
        if matched:
            my_man = searchPlayer(matched.group(1), matched.group(2), players)
        else:
        ...
        if len(my_man) == 0:
            atptour_name, atptour_id = scrapPlayerId(full_name)
            if atptour_name != None and atptour_id != None:
...
def getPlayerIds(players_in_matches_dataset):
    ...
    winner_atp_ids = [matchPlayer(row[0], row[1], players, player_ids, new_players_to_scrap_ids) for row in players_in_matches_dataset.to_numpy()]
    loser_atp_ids = [matchPlayer(row[2], row[3], players, player_ids, new_players_to_scrap_ids) for row in players_in_matches_dataset.to_numpy()]
    
    #player_atp_ids = [matchPlayer(row[0], row[1], players, player_ids, new_players_to_scrap_ids) for row in players_in_matches_dataset.to_numpy()]
...
def retrieveMissingId(player_id, atptour_id, player_ids_manual_collect):
...
def retrieveMissingIds(dataset):
    ...
    p1_ids_dataframe = dataset.apply(lambda row: retrieveMissingId(row["winner_id"], row["p1_id"], player_ids_manual_collect), axis=1)
    p2_ids_dataframe = dataset.apply(lambda row: retrieveMissingId(row["loser_id"], row["p2_id"], player_ids_manual_collect), axis=1)
    
    return (p1_ids_dataframe, p2_ids_dataframe, player_ids_manual_collect)

def scrapPlayers(players_ids):
    #TODO
    return