...
def scrapTournamentsByYear(year):
    ...
    for element in elements:
        ...
        level = re.search("categorystamps_(.+)\.", img_src)
        if level == None:
    ...
    tournaments = pd.concat([pd.Series(ids), pd.Series(names), pd.Series(formatted_names), pd.Series(cities), pd.Series(countries), 
                             pd.Series(levels), pd.Series(number_of_competitors),
                             pd.Series(surfaces)], axis=1)
    tournaments.columns = ["id", "name", "formatted_name", "city", "country", "level", "number_of_competitors", "surface"]

    return tournaments

def recordTournamentsByYear(year):
    tournaments = scrapTournamentsByYear(year)
...
def scrapTournamentId(name, year, tournaments, new_tournaments_to_scrap):
    driver = webdriver.Chrome('/home/davy/Drivers/chromedriver')
    ...
    if tournaments_scrapped == None:
        url  = 'https://www.atptour.com/-/ajax/Scores/GetTournamentArchiveForYear/{}'.format(year - 1)
        driver.get(url)
        time.sleep(1)
        try:
            content = driver.find_element_by_xpath("//pre").text
...
def matchTournament(name, year, tournaments, new_tournaments_to_scrap):
    if(name.startswith("Davis Cup")):
        name = "Davis Cup"
    if name not in tournaments:
        tourn_id = scrapTournamentId(name, year, tournaments, new_tournaments_to_scrap)
        return tourn_id
    else:
        return tournaments[name]
    
def getTournamentsIds(dataset):
    ...
    tournaments_ids = [matchTournament(row[0], str(row[1])[:4], tournaments, new_tournaments_to_scrap) for row in dataset.reindex(index=dataset.index[::-1]).to_numpy()]
    
    print("---getTournamentsIds  %s seconds ---" % (time.time() - start_time))
    return (tournaments_ids[::-1], new_tournaments_to_scrap)


def retrieveMissingTournamentId(tournament_name, atptour_id, tournament_ids):
    if atptour_id == -1:
...
def retrieveMissingTournamentIds(dataset):
    

    tournament_ids_manual_collect = pd.read_csv("../datasets/tournament_ids_matching_manual_collect.csv", index_col="name")
    
    tournament_ids_dataframe = dataset.apply(lambda row: retrieveMissingTournamentId(row["tourney_name"], 
                                             row["tournament_id"], tournament_ids_manual_collect), axis=1)
    
    return (tournament_ids_dataframe, tournament_ids_manual_collect.values.tolist())

def scrapTournament(tournament_id, tournament_formatted_name, year):
    driver = webdriver.Chrome('/home/davy/Drivers/chromedriver')
    match_url  = 'https://www.atptour.com/en/tournaments/{0}/{1}/overview'.format(tournament_formatted_name, tournament_id)
    driver.get(match_url);
    time.sleep(1) # Wait 1 sec to avoid IP being banned for scrapping
    ...
    try:
        ...
        level = None
        try:
            level = re.search("categorystamps_(.+)_", img_src).group(1)
        except:
            pass
        ...
        tournament = Tournament(tournament_id, name, tournament_formatted_name, city, country, surface, number_of_competitors, level)
    except Exception:
        match_url  = 'https://www.atptour.com/en/scores/archive/{0}/{1}/{2}/results'.format(tournament_formatted_name, tournament_id, year)
        driver.get(match_url);
        time.sleep(1) # Wait 1 sec to avoid IP being banned for scrapping
        try:
            ...
            try:
                ...
                img_src = img.get_attribute("src")
                level = re.search("categorystamps_(.+)\.", img_src).group(1)
            except:
                pass
            
            number_of_competitors = int(driver.find_element_by_xpath("//td[@class='tourney-details-table-wrapper']/table/tbody/tr/td[1]/div[2]/div/a[1]/span").text)
            
            surface = driver.find_element_by_xpath("//td[@class='tourney-details-table-wrapper']/table/tbody/tr/td[2]/div[2]/div/span").text
            
            tournament = Tournament(tournament_id, name, tournament_formatted_name, city, country, surface, number_of_competitors, level)
        
        except Exception:
            print("Couldn't find tournament '{0}' with id {1} in {2}".format(tournament_formatted_name, tournament_id, year))
    
    driver.quit()
    
    return tournament

def scrapTournaments(tournaments_info):
    tournaments = []
    for tournament_info in tournaments_info:
        tournament = scrapTournament(tournament_info[0], tournament_info[1], 
...
def recordTournaments(tournaments):
    tournaments_json = getTournamentsJSON(tournaments)
    myclient = pymongo.MongoClient(MONGO_CLIENT)
    mydb = myclient["tennis"]
    mycol = mydb["tournaments"]        
